(function () {
  ...
  async function startExperience() {
    ...
    // تشغيل مؤقت احتياطي (Timeout) لمدة 6 ثواني
  ...
  // دالة الإرسال لـ Formspree باستخدام Fetch لعدم الخروج من الصفحة
  async function handleRsvpSubmit(event) {
    ...
    formData.append("الاسم", guestNameField.value.trim());
    formData.append("الحضور", attendanceField ? attendanceField.value : "نعم، بكل سرور");
    ...
    try {
      ...
      if (response.ok) {
        setFeedback(formSuccess, "شكراً لك. تم تأكيد الحضور بنجاح!");
        rsvpForm.reset();
        updateGuestCount(1);
        renderGuestPass({
